...
def plot_trends(input_path, output_path, file_trend_name, xlim1, xlim2, ylim1, ylim2,
                nmaster, plot_name, plot_choice, scale_tendency, merge_tag=1, plot_tag=1):
    ...
    if merge_tag: 
        ...
        for i in range(len(chunks_final)-1):
            ...
            v = temp_store_trend
            for ii in range(4):
                print('pct not NaN / min / max:', f'{np.count_nonzero(v[:,:,ii]==0)/v[:,:,ii].size:.3f}', np.nanmin(v[:,:,ii]), np.nanmax(v[:,:,ii]))
